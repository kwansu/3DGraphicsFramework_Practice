...
cObject_Collider* cObject_Cube::GetCollider()
{
	if (m_pCollider)
		return m_pCollider;
...
bool cObject_Cube::IsPicked(cVec3* pvDir, cVec3* pvPos)
{
	int size = m_aVertexPN.size();
	for (int i = 0; i < size; i += 3)
	{
		if (D3DXIntersectTri(&m_aVertexPN[i].p
			, &m_aVertexPN[i + 1].p, &m_aVertexPN[i + 2].p
			, pvPos, pvDir, NULL, NULL, NULL))
		{
			return true;
		}
	}
...
void cObject_Cube::MoveObject(cVec3* pvDir, cVec3* pvPos, cVec3* pvNormal)
{
