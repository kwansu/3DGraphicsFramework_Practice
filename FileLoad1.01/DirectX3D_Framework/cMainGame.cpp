...
void cMainGame::Initialize()
{
	...
	m_pCamera = g_pCamera;
	m_pGrid = new cGrid;
	m_pColl = new cObject_Mario;
	m_pCollBottom = new cObject_Collider_Bottom;
	//m_pMap = new cObject_Map;
	g_pObjects->Setup();

	m_pCamera->SetTarget(&m_pColl->m_vPos);

	m_pColl->Setup(m_pCollBottom);
	m_pCamera->Setup();
	m_pGrid->Setup();
	...
	m_light.Ambient = xColor(1.0f, 1.0f, 1.0f, 1.0f);
...
void cMainGame::Render()
{
	...
	m_pGrid->Render();
	//m_pMap->Render();
	if (g_pInput->IsKeyPress('O'))
	{
		g_pDevice->SetRenderState(D3DRS_LIGHTING, false);
		m_pCollBottom->Render();
	}
	else
	{
